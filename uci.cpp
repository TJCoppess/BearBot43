...
int main() {
	...
	while (isRunning) {
		...
		if (commandSegments[0] == "uci") {
			...
			// cout << "info new game initialized" << endl;
			...
			if (commandSegments.size() >= 2 && commandSegments[1] == "wtime"){} //board.setWTime(std::stoi(commandSegments[2]));
			if (commandSegments.size() >= 4 && commandSegments[3] == "btime"){} //board.setBTime(std::stoi(commandSegments[4]));
			if (commandSegments.size() >= 6 && commandSegments[5] == "winc"){}  //board.setWInc(std::stoi(commandSegments[6]));
			if (commandSegments.size() >= 8 && commandSegments[7] == "binc"){}  //board.setBInc(std::stoi(commandSegments[8]));
